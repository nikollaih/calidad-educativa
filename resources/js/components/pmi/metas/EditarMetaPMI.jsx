...
const CreateMetaPMI = ({ agregarUrl = '', csrfToken = '', meta = {} }) => {
    // Estado para la meta (evita conflicto con prop meta)
    ...
    useEffect(() => {
        if (meta && meta.id) {
            ...
            const parsedIndicadores = (meta.indicadores || []).map(indicador => ({
                descripcion: indicador.descripcion,
                actividades: (indicador.actividades || []).map(actividad => ({
                    descripcion: actividad.descripcion,
                    peso: parseFloat(actividad.peso)
                }))
            }));

            setIndicadores(parsedIndicadores.length > 0 ? parsedIndicadores : [{
                descripcion: '',
                actividades: [{ descripcion: '', peso: 100 }]
            }]);
        }
    }, [meta]);
    ...
    const handleActividadChange = (indicadorIndex, actividadIndex, e) => {
        const { name, value } = e.target;
        const newIndicadores = [...indicadores];
        const pesoValue = name === 'peso' ? Math.min(100, Math.max(0, Number(value) || 0)) : value;
    ...
    const addActividad = (indicadorIndex) => {
        ...
        if (pesoRestante <= 0) {
            ...
            actividades.forEach(act => {
                act.peso = Math.round(factor * 10) / 10;
            });
        }
    ...
    return (
        <div className="container py-4">
            <form method="POST" action={agregarUrl}>
                ...
                {/* Datos ocultos para enviar indicadores y actividades */}
                ...
                <h3 className="mb-4">
                    {meta?.id ? 'Editar Meta PMI' : 'Crear Nueva Meta PMI'}
                </h3>
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-6 mb-3">
                                ...
                                <input
                                    ...
                                    value={metaData.valor_requerido}
                                    onChange={handleMetaChange}
                                    required
                                />
                            </div>
                ...
                <button type="submit" className="btn btn-success" disabled={!pesoValido}>
                    Guardar Meta
                </button>
            </form>
        </div>
