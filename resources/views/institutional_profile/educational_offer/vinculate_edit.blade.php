...
<div class="container">
    ...
    <form id="vinculationForm" action="{{ route('educational-offer.make-vinculation', $sedeEducational->sede_id) }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <select class="form-select" name="educational_levels[]" id="select2Multiple" multiple>
                @foreach($educationalLevels as $educationalLevel)
                    <option value="{{ $educationalLevel->id }}"
                            {{ $sedeEducational->educationalLevels->whereIn('id', $educationalLevel->id)->first() ? 'selected' : '' }}>
                        {{ $educationalLevel->full_hierarchy }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class=" mb-4">
            ...
            <div class=" card card-body">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" name="schedule[name]" id="schedule" required>
                        @foreach($educationalSchedules as $key => $value)
                            <option value="{{ $key }}"
                            {{ $sedeEducational->schedule->name == $key ? 'selected' : '' }}
                                >{{ $value }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="schedule[schedule]" id="schedule_description" placeholder="Ej: Horario matutino de 8 AM a 12 PM" value="{{$sedeEducational->schedule->schedule}}" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="schedule[notes]" id="full_description" rows="4" value={{$sedeEducational->schedule?->notes }} placeholder="Detalles adicionales sobre el horario y estructura educativa..."></textarea>
                </div>
                 <!-- Adjuntar archivo -->
                <div class="mb-4">

                    <label for="attachment" class="form-label fw-bold">Adjuntar Anexo de horario</label>
                    @if(isset($sedeEducational->schedule->anexo))
                        <a href="{{ $sedeEducational->schedule?->anexo?->url }}" target="_blank" class="btn btn-outline-info btn-sm">
                            <i class="fas fa-eye"></i> Ver anexo Actual
                        </a>
                    @endif
